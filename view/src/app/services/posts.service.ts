...
export class PostsService {
  ...
  getAll(limit: number = 25, page: number = 0): Observable<IRssResponse> {
    return this.http.get<IRssResponse>('http://localhost:5000/api/v1/posts/', {
      params: new HttpParams({
        fromObject: {limit: limit, offset: limit * page}
      })
    }).pipe(
      retry(5),
      tap(posts => {this.posts = posts.message; console.log(posts)}),
  ...
  create(post: ISendPost): Observable<IRssResponse> {
    ...
    return this.http.post<IRssResponse>('http://localhost:5000/api/v1/posts/', post, {
      headers: {
        'Authorization': `Bearer ${this.cookieService.getCookie('token')}`
      }
    })
      .pipe(
        tap(posts => posts.message.forEach((post) => this.posts.push(post)))
      )
  }
